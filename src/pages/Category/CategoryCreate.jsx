...
const CategoryCreate = () => {
    ...
    const onSubmit = async () => {
        ...
        try {
            const res = await categoriesApi.create(result);
    ...
    return (
        <Layout>
            ...
            <Grid container spacing={4}>
                {languages.map((item, index) => (
                    <Grid key={index} item xs={12}>
                        <Controller
                            ...
                            rules={{
                                required: t('category_create.required_message')
                            }}
                            defaultValue=""
                            render={({ field }) => (
                                <TextField
                                    {...field}
                                    variant="outlined"
                                    label={`${t('category_create.name_field')} (${item.name})`}
                                    sx={{ width: '50%' }}
                ...
                <Grid item xs={12}>
                    <Button type="submit" onClick={handleSubmit(onSubmit)} variant="contained">{t('category_create.save_button')}</Button>
                </Grid>
            </Grid>
            <Backdrop
